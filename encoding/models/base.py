...
class BaseNet(nn.Module):
    def __init__(self, nclass, backbone, aux, se_loss, jpu=False, dilated=False, norm_layer=None,
        ...
        if backbone == 'resnet18':
            self.pretrained = resnet.resnet18(pretained=True, dilated=dilated, norm_layer=norm_layer, root=root)
        elif backbone == 'resnet34':
            self.pretrained = resnet.resnet34(pretrained=True, dilated=dilated, norm_layer=norm_layer)
        
        elif backbone == 'vgg16':
            self.pretrained = vgg16(pretrained=True).features
